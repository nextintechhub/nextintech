...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        if ($user) {
            ...
            // Remove old reset requests for this user
            ...
            $message = "<div class='alert success'>✅ Reset link generated! <br> <a href='$resetLink' style='color:#fff;'>$resetLink</a></div>";
        } else {
  ...
  <style>
    * {margin:0; padding:0; box-sizing:border-box; font-family:'Poppins',sans-serif;}
    ...
    .topbar .top-right a {
      ...
      background:#e6bb91; padding:6px 12px; border-radius:8px; transition:0.3s;
    }
    ...
    input {
      width:100%; padding:12px; border:none; outline:none; border-radius:10px;
      font-size:14px; background:#fff; color:#000;
    }
    .btn-row { display:flex; justify-content:space-between; gap:10px; margin-top:18px; }
    .btn {
      flex:1; text-align:center; padding:12px; border-radius:12px; font-weight:600;
      cursor:pointer; transition:0.3s; border:none; text-decoration:none;
    }
